...
const useLocalStorage = () => {
  const setLocalStorage = <T extends keyof LocalStorageItems>(
    ...
    try {
      ...
      localStorage.setItem(`streamdevices:${key}` as string, value as string);
    } catch (error) {
  ...
  const getLocalStorage = <T extends keyof LocalStorageItems>(
    ...
    try {
      let value = localStorage.getItem(`streamdevices:${key}` as string);
      if (value === null) {
        return null;
      }
      try {
  ...
  const removeLocalStorage = (key: keyof LocalStorageItems) => {
    localStorage.removeItem(`streamdevices:${key}` as string);
  };
